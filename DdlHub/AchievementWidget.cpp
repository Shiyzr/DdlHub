...
void AchievementWidget::paintEvent(QPaintEvent *event) {
    ...
    if (!icon.isNull()) {
        ...
        if (completed) {
            ...
            QImage grayWithAlpha = icon.toImage().convertToFormat(QImage::Format_ARGB32);
            for (int y = 0; y < grayWithAlpha.height(); ++y) {
                for (int x = 0; x < grayWithAlpha.width(); ++x) {
                    QColor color = grayWithAlpha.pixelColor(x, y);
                    int alpha = color.alpha();
                    int gray = qGray(color.rgb());  // 使用 qGray 保留亮度信息
                    grayWithAlpha.setPixelColor(x, y, QColor(gray, gray, gray, alpha));
                }
            }
            displayIcon = QPixmap::fromImage(grayWithAlpha);
        }
    }
